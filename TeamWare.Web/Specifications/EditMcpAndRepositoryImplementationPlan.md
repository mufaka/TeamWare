...
|-------|------------|--------|
| 51 | Edit Agent Repositories and MCP Servers | Complete |
...
- [x] Add `ClearAccessToken` (bool) to `SaveAgentRepositoryDto`
...
- [x] Add `ClearAuthHeader` (bool) and `ClearEnv` (bool) to `SaveAgentMcpServerDto`
  - Same three-way semantics for each field
- [x] Update `UpdateRepositoryAsync` in `AgentConfigurationService` to apply keep-current logic for `EncryptedAccessToken`
- [x] Update `UpdateMcpServerAsync` in `AgentConfigurationService` to apply keep-current logic for `EncryptedAuthHeader` and `EncryptedEnv`
- [x] Verify `AddRepositoryAsync` and `AddMcpServerAsync` are unaffected (new entries with no secret are already handled correctly)
- [x] Add unit tests in `AgentConfigurationServiceTests`:
...
- [x] Wrap the Repositories `<table>` in a parent `<div x-data="{ editingId: null }">` to hold single-open-at-a-time state
- [x] Add an **Edit** button to each repository row's Actions cell (alongside the existing Remove button)
  - `@@click` toggles `editingId` between the row's `@repo.Id` and `null`
  - Button label changes to "Cancel" when that row is open
- [x] Add an Alpine.js-controlled `<tr>` immediately after each data row:
...
- [x] Edit form posts to `UpdateAgentRepository` and includes:
...
- [x] Add integration tests in `AdminControllerAgentConfigTests`:
...
- [x] Wrap the MCP Servers `<table>` in a parent `<div x-data="{ editingId: null }">` to hold single-open-at-a-time state
- [x] Add an **Edit** button to each MCP server row's Actions cell (alongside the existing Remove button)
...
- [x] Add an Alpine.js-controlled `<tr>` immediately after each data row:
  ...
  - Contains a single `<td colspan="5">` wrapping the edit form
- [x] Edit form posts to `UpdateAgentMcpServer` and includes:
